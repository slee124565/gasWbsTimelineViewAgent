...
This document details the functionality and automation rules for the `gasWbsTimelineViewAgent` script. This version refines the Web API interface and aligns documentation with the latest code.
...
The script can be deployed as a Google Apps Script Web App to provide an external API for reading WBS data.

### 4.1. `doGet(e)` - Read Sheet Data
*   **Purpose**: Handles HTTP GET requests to read data from a specified WBS sheet and return it in JSON format. The API is designed to only return rows where the **Object** column (Column A) contains text, effectively filtering out empty or unused rows.
*   **Endpoint**: To use this API, you must first deploy the script as a Web App.
    1.  In the Apps Script editor, click **Deploy > New deployment**.
    2.  Select **Web app** as the deployment type.
    3.  Configure the access permissions under **Who has access** (e.g., "Anyone" for a public API, or a more restrictive setting).
    4.  After deployment, you will receive a unique Web App URL (`.../exec`). This URL is your API endpoint.
*   **URL Parameters**:
    *   `sheetName` (optional): Specifies the name of the sheet to read. If omitted, it defaults to `wbs`.
    *   **Example**:
        *   `.../exec` (reads from the `wbs` sheet)
        *   `.../exec?sheetName=wbs-2` (reads from the `wbs-2` sheet)
*   **Output Format**: The API returns a JSON object with a `status` field, the `sheet` name, and a `data` array. Each element in the `data` array is a JSON object representing a row, with keys derived from the sheet's header row.
      ...
      "data": [
        {
          ...
          "TaskDescription-1": "Configure project environment",
          "StartDate": "2024-01-10T00:00:00.000Z",
          "WorkDays": 2,
          "Resource": "Alice",
          "TaskStatus": "Done",
          "DoneDate": "2024-01-12T00:00:00.000Z",
          "DueDate": "2024-01-12T00:00:00.000Z",
          "TaskDescription-2": "[Alice]-Configure project environment"
        }
      ]
    }
    ```

*   **Example Error Response** (if sheet is not found):
    ```json
    {
      "status": "error",
      "message": "Sheet "wbs-nonexistent" not found."
    }
    ```

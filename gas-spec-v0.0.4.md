# Google Apps Script Specification: WBS Automation v0.0.4

This document details the implemented functionality and automation rules for the `gasWbsTimelineViewAgent` script, reflecting the state of `code.js` after the function merge and rename. This version introduces a consolidated content and formula reset feature.
...
2.  **重設任務內容與公式 (保留首欄) (Reset Task Content & Formulas (Keep First Column))**: Resets the current WBS sheet's content and repairs its automated formulas, making it ideal for reuse as a template.
...
*   **Purpose**: This consolidated function, triggered from the custom menu, clears the active WBS sheet of all task-related data **and** resets the automated formulas. This is the primary tool for reusing a WBS sheet or fixing broken formulas.
*   **Execution**:
    ...
    3.  Re-applies the correct formulas to the `DueDate` (Column I) and `TaskDescription-2` (Column J) columns for all rows, ensuring any accidental deletions or modifications are corrected.
    4.  **Crucially, this action preserves**:
...
|---|---|---|---|
| A | **Object** | String | **Manual Input:** The name of the project or target. **(Preserved by `resetWBSContentFormulas`)** |
...
| I | **DueDate** | Date | **Automated by Sheet Formula:** The cell contains the formula: `=IF(AND(D2<>"", E2<>""), WORKDAY(D2, E2, 'holidays-tw'!A$2:A), "")`. This formula is reapplied by the `resetWBSContentFormulas` function. |
| J | **TaskDescription-2**| String | **Automated by Sheet Formula:** The cell contains the formula: `=IF(C2<>"", IF(F2<>"", "["&F2&"]-"&C2, "[未指派]-"&C2), "")`. This formula is reapplied by the `resetWBSContentFormulas` function. |
